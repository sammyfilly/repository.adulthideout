...
def process_motherless_content(url):
    ...
    if "motherless://" in url or "shouts" in url:
        ...
        if all(term not in url for term in ["term", "viewed", "porn", "/gv"]):
            url = f"{url}/videos/recent"
...
def add_basic_dirs():
    ...
    for name, url, mode in basic_dirs:
        add_dir(name, url, mode, f'{logos}motherless.png', fanart)
...
def process_category_paths(current_url):
    ...
    if "/live/videos" in current_url:
        add_dir(
            '[COLOR yellow]Reload[/COLOR]',
            'https://motherless.com/live/videos',
            2,
            f'{logos}motherless.png',
            fanart,
        )
    for path, name in category_paths:
        url = f"https://motherless.com{path}"
        add_dir(name, url, 2, f'{logos}motherless.png', fanart)
...
def process_motherless_categories(url):
    ...
    for urls in category_dict.values():
        for url in urls:
            formatted_name = format_category_name(url)
            add_dir(
                formatted_name,
                f'https://motherless.com/porn/{url}/videos',
                2,
                f'{logos}motherless.png',
                fanart,
            )
...
def process_motherless_groups(url):
    ...
    for thumb, url, name in match:
        ...
        url = f'https://motherless.com/gv/{url}'
        add_dir(name, url, 2, f"http://{thumb}", fanart)
    add_next_button(content)
...
def add_next_button(content):
    try:
        if match := re.compile('<link rel="next" href="(.+?)"/> ').findall(
            content
        ):
