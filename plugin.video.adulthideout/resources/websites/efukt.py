...
def process_efukt_content(url, page=1):
    xbmc.log(f"Entering process_efukt_content with URL: {url}", xbmc.LOGINFO)
    if page == 1:
        add_dir('Search Efukt', 'efukt', 5, f'{logos}efukt.png', fanart)
...
def play_efukt_video(url):
    ...
    xbmc.log(f'Resolved media URL: {media_url}', xbmc.LOGINFO)
    return media_url
